...
The one accepted exception to "CLI-only, no GUI": the `mashup-ui` subcommand (`soundweave/ui/`) is a local, stdlib-only browser front-end — bound to `127.0.0.1` only, no new dependency — for both `mashup` (`/`) and `loop` (`/loop`). It doesn't reimplement pipeline logic; submitting either form shells out to `python -m soundweave mashup ...` or `python -m soundweave loop ...` (the same commands you'd type by hand) as a subprocess and streams the log back. Both forms share one job-tracking mechanism in `server.py` (`_MashupUIServer._launch_job`): a job's directory is created before the subprocess starts (so uploads/urls.txt have somewhere to land and the log can stream), but is deleted after a non-zero exit — a failed run leaves no output folder behind, only the in-memory log tail for the browser's last poll. Treat this as the established pattern if another subcommand ever wants a browser front-end — a thin process-launching wrapper, not a second pipeline implementation.

`mashup` also has an optional `--loop-count N` (surfaced in the mashup-ui form as "Repeat count"): after crossfading the URLs into one track, repeat that whole track N times with a gap, by constructing a `LoopConfig` and calling `loop_stage()` directly on the crossfaded file — the exact same stage function the standalone `loop` subcommand uses on a user-supplied file, not a reimplementation. Not supported together with `--images` (per-track image sequencing has no concept of a repeat/gap-padded sequence yet); `--image` and `--animated-background` both self-probe the (possibly looped) audio's duration so they need no special-casing.
