...
**Status**: done, unmerged. Fixed and verified in worktree branch `worktree-agent-ad495494f4047d5bb`, commit `304d17d` (2026-07-25) — root cause confirmed exactly as hypothesized below, fix verified with real numbers (see session record), loop seam still clean. Sitting on its own branch pending `/code-review` + merge decision, not yet on `main`. Original spec kept below for reference.

**Found** 2026-07-25 immediately after `flower_sway.py` shipped (`ceab79d`) — the sway motion is real and correctly-scaled (see Addendum 3 above), but has a visible bug: a swayed flower leaves a duplicate behind at its original position instead of clearing it.
...
**Status**: attempted, unmerged, real defect found — needs a decision, not more unattended work. Implemented in worktree branch `worktree-agent-ae5da36adb9f24468`, commit `77f9611` (2026-07-25). The height-gradient technique below works exactly as designed (anchor grad ≈0 at each component's base, 1.0 at its own canopy top, no duplication artifact — verified numerically). But honest visual inspection of the real rendered video found a genuine artifact the design didn't anticipate: horizontal streaking/tearing in the background (mountain rock texture) around foliage edges against detailed backgrounds — right-side trees against flat sky look convincing, left-side treeline against detailed rock does not. Root cause: the gradient-field margin extension (nearest-true-foliage-pixel distance) is spatially incoherent for jagged/gappy silhouettes, invisible against flat sky but visible against high-contrast detail. Committed anyway per the user's own "report honestly, don't force a bad result through" instruction, rather than silently shipping or silently discarding it. **Next step is a human call**: merge as "trees sway with a known limitation," scope the effect off high-detail background regions, attempt a fix to the margin-extension math (e.g. smooth the extended gradient instead of nearest-pixel lookup), or drop it and keep only flowers swaying. Original spec kept below for reference.

**Scoped** 2026-07-25. User explicitly asked for bushes/trees to sway too (not just flowers) — and explicitly asked to be told plainly if this can't look natural rather than have a bad-looking attempt pushed through. Assessment below is that answer: **do not reuse flower_sway.py's uniform-shift technique for this** — it will look artificial for these subjects. This needs a different, harder technique from the start, not a parameter tweak on the flower approach.
...
### T5: FFmpeg progress reporting (percentage/ETA) for long-running stages

**Status**: not started. Proposed 2026-07-25 in response to "what would make this a better experience" — `run_ffmpeg()` currently gives no feedback between "Rendering..." and completion. For an hour-long mashup that's 20+ minutes of silence, in both the CLI and `mashup-ui`'s polled log stream.

**Owns**: `soundweave/ffmpeg/executor.py` (`run_ffmpeg` — parse ffmpeg's progress output). Callers (`merge_stage`, video stages, MP3 encode) pass an already-known `total_duration_s` where they have one; no change to `run_ffmpeg`'s return value or error contract.

**Scope**: add `-progress pipe:1` to ffmpeg invocations and parse the `key=value` stream (`out_time_ms`, `speed`) to log a periodic percentage/ETA line during long commands. Make it opt-in via an optional `total_duration_s` param on `run_ffmpeg()` so callers without a known duration (e.g. the merge stage's crossfade chain, if duration isn't cheaply known upfront) are unaffected.

**Out of scope**: a real terminal progress bar (curses etc.) — periodic log lines are enough, and `mashup-ui` gets them for free through its existing 1s log poll once `run_ffmpeg` emits them.

**Acceptance**: a real run with a multi-minute audio duration prints periodic progress lines (not just start/end) during merge/video rendering; existing `run_ffmpeg` callers/tests are unaffected when `total_duration_s` is omitted.

### T6: mashup-ui parity — expose --images and --animated-background

**Status**: not started. Gap found by `/code-review` immediately after `--animated-background` shipped (2026-07-25): the `mashup-ui` form only has an `--image` field; `--images` and `--animated-background` (both valid `mashup` flags) are unreachable from the browser UI, even though `server.py`'s own docstring describes it as running "the same `mashup` subcommand you'd run by hand."

**Owns**: `soundweave/ui/templates.py` (additive — new form fields/inputs), `soundweave/ui/server.py` (additive — handle the new fields in `start_job()`, append the matching CLI flag).

**Scope**: let the form choose one of the three mutually-exclusive video sources (matching the CLI's `argparse` group): single image upload (existing), per-track images (new — multi-file upload, natural-sorted like the CLI's directory scan), animated background video upload (new — single file). `start_job()` appends `--image`/`--images`/`--animated-background` accordingly, same pattern as the existing `--image` wiring.

**Out of scope**: any change to the underlying CLI/mashup pipeline — purely making the existing three modes reachable from the UI.

**Acceptance**: submitting the form with each of the three video-source choices produces the correct `soundweave mashup` subprocess invocation (verify by inspecting the spawned command, same approach as `server.py`'s existing curl-based verification).

### T7: single entry point from a scene image to an --animated-background-ready video

**Status**: not started. Proposed 2026-07-25 — today, going from "I have a scene.png" to a usable `--animated-background` input means manually running several `tools/ambient_bg/` scripts (`depth_render.py`, optionally `flower_sway.py`/`foliage_sway.py`, `particles.py`, `composite.sh`) in the right order with matching `--duration`/`--scene` flags by hand.

**Owns**: `tools/ambient_bg/` only (e.g. a new `run_all.sh` or `pipeline.py` orchestrator). Does not touch the `soundweave` package — `tools/ambient_bg/` stays standalone per T2's original placement decision.

**Scope**: one script that runs the existing, independently-tested stages in order, passing a single set of user-facing flags (`--scene`, `--duration`, `--output`) through to each stage's own existing CLI rather than reimplementing their logic. Each stage must remain independently runnable for tuning, per T2's original design goal — this is additive convenience on top, not a replacement.

**Out of scope**: wiring this into the `soundweave` CLI/mashup pipeline itself (still a separate, later task per T2's placement decision — this tool is heavy/GPU-dependent/exploratory, not ready to be a core dependency); changing any individual stage script's own logic. Should be sequenced after T3/T4 are resolved (merge, fix, or drop) so it orchestrates the settled version, not a known-buggy one.

**Acceptance**: a single command against a fresh scene image produces a finished, composited `final.mp4` with no manual intermediate steps; each stage's standalone re-run capability is unaffected.

### T8: --dry-run for mashup (resolve tracklist without downloading/encoding)

**Status**: not started. Proposed 2026-07-25 — for a `urls.txt` with many tracks, a mashup run commits to a full download + crossfade + encode before the user sees the resolved track order, titles, or estimated total duration; a mistake (wrong URL, bad order) is only caught after a long run.

**Owns**: `soundweave/cli.py` (additive — new `--dry-run` flag on `mashup`), `soundweave/stages/download.py` (may need a metadata-only probe path via yt-dlp's `--dump-json`/`--skip-download` rather than a real download).

**Scope**: with `--dry-run`, resolve each URL's title/duration via yt-dlp metadata only (no audio download), print the resolved tracklist (title, duration, order) and estimated total runtime, then exit before touching merge/encode/video stages.

**Out of scope**: caching/reuse of dry-run metadata by a subsequent real run — `download_stage`'s own cache already handles that independently; dry-run is read-only/informational only.

**Acceptance**: `soundweave mashup --urls urls.txt --output output --dry-run` prints the full resolved tracklist and estimated duration without creating files in `--output` or downloading audio, and exits with the normal exit-code contract.
